//Reference Discramer
console.log("BW Japan", "v20220110.3");
		...
		if(!/[0-9a-f]{8}\-(?:[0-9a-f]{4}\-){3}[0-9a-f]{12}/.test(bid)) {
						...
						onload: async function(reS) {
							...
							while(!auuid && _detail$retry_ < 20) {
								...
								try {
									...
									if(!tar.length) tar = Array.from(html.querySelectorAll("a[title*='"+ non.toHalfWidth({oswin:1,space:1,prepend:presym}) +"']")).filter(v => !/(期間限定|お試し|試し読み)/.test(v.title));
									if(tar.length == 1) {
			...
			GM.xmlHttpRequest({
				...
				onload: function(res) {
					...
					fn = (autag + (on || bn)).toHalfWidth({oswin:1,space:1,prepend:presym});
					bd.fileName = fn;
			...
			c.toBlob(async(v)=>{
				...
				if(startf) {
					...
					if(!wait && img$size[curp] > 20000) { //Detail collect will only do once and Cover should be larger than 20KB
						...
						num = num.toHalfWidth({oswin:1,space:1,prepend:presym});
